"""HTTP routes for appointments-service.

Generic CRUD over the `appointments` table. Every mutating endpoint publishes a
domain event (appointment.booked) and an audit event; reads publish audit only.
...
CREATED_EVENT = "appointment.booked"
UPDATED_EVENT = "appointment.cancelled"
...
def build_blueprint(svc) -> Blueprint:
    ...
    @bp.post("/<int:record_id>/cancel")
    @require_auth(scopes=[f"{RESOURCE}.write"])
    def cancel(record_id: int):
        row = db.query_one(f"UPDATE {TABLE} SET status='cancelled', updated_at=now() WHERE id=%s RETURNING *",
                           (record_id,))
        if not row: return jsonify({"error": "not found"}), 404
        bus.publish("appointment.cancelled", key=str(record_id), value=_row_json(row))
        emit_audit(bus, action="appointment.cancel", actor=_actor(), target=f"appointment:{record_id}")
        return jsonify(_row_json(row))

    @bp.post("/<int:record_id>/check-in")
    @require_auth(scopes=[f"{RESOURCE}.write"])
    def check_in(record_id: int):
        row = db.query_one(f"UPDATE {TABLE} SET status='checked_in', updated_at=now() WHERE id=%s RETURNING *",
                           (record_id,))
        if not row: return jsonify({"error": "not found"}), 404
        emit_audit(bus, action="appointment.check_in", actor=_actor(), target=f"appointment:{record_id}")
        return jsonify(_row_json(row))
